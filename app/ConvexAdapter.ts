...
function callQuery<Query extends FunctionReference<"query">>(
  query: Query,
  args: Omit<FunctionArgs<Query>, "secret"> & Record<string, unknown> // Specify type explicitly
) {
...
function callMutation<Mutation extends FunctionReference<"mutation">>(
  mutation: Mutation,
  args: Omit<FunctionArgs<Mutation>, "secret"> & Record<string, unknown> // Specify type explicitly
) {
  return fetchMutation(mutation, addSecret(args) as FunctionArgs<Mutation>);
}

if (process.env.NEXT_PUBLIC_CONVEX_AUTH_ADAPTER_SECRET === undefined) {
...
function addSecret<T extends Record<string, unknown>>(
  ...
  return {
    ...args,
    secret: process.env.NEXT_PUBLIC_CONVEX_AUTH_ADAPTER_SECRET!,
  };
}
